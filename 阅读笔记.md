...
2. 通过pt来适配，要使ptShowing / designedWidth = pxShowing / widthPixels，通过上面的android pt转px的计算公式，将`DisplayMetrics.xdpi`设为**72 * `DisplayMetrics.widthPixels`/1080**，这里我们选择按照1080px的设备来适配。再使用pt，这样系统在将pt转为px时即可将控件宽度按照1080px适配，即在不同像素大小的设备上，显示的大小与1080设备相同。而且此方案没有对文字大小的计算进行干扰，不需要再对sp进行适配。  
