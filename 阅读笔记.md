...
&emsp;&emsp;以横竖屏切换为例，当activity以横/竖屏启动时，activity的声明周期:   

&emsp;&emsp;onCreate()   ->   onStart()  ->  onResume()  

&emsp;&emsp;当切换为竖/横屏时，activity的生命周期：  

&emsp;&emsp; onPause -->onSaveInstanceState -->onStop -->onDestroy -->onCreate-->onStart -->onRestoreInstanceState-->onResume -->onPause -->onStop -->onDestroy
所以一般的横竖屏的切换实际上是销毁原有的activity并重新创建。  
&emsp;&emsp;但是当我们在AndroidManifest.xml中声明android:configChanges属性值为`orientation|screenSize`时，横竖屏切换将不会调用上述任何生命周期的方法，而是直接调用onConfigurationChanged()。其中orientation代表横竖屏切换，screenSize代表屏幕大小发生了改变。  
&emsp;&emsp; 当然，一般大多数activity直接在AndroidManifest中的android:screenOrientation属性指明是横屏(landscape)还是竖屏(portrait)。
